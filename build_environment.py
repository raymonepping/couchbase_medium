...
def connect_to_cluster():
    ...
    auth = PasswordAuthenticator(
        os.getenv("COUCHBASE_USERNAME"), os.getenv("COUCHBASE_PASSWORD")
    )
    cluster = Cluster(
        f'couchbase://{os.getenv("COUCHBASE_ENDPOINT")}', ClusterOptions(auth)
    )
...
def create_bucket(cluster):
    ...
    try:
        ...
        logger.info(
            'Bucket "%s" created with RAM quota %d MB.', BUCKET_NAME, RAM_QUOTA_MB
        )
...
def create_scope_and_collections(collection_manager, scope, collections):
    ...
    for collection in collections:
        try:
            ...
            logger.info(
                'Collection "%s" already exists in scope "%s".', collection, scope
            )
...
def create_indexes(cluster):
    ...
    for primary in INDEXES.get("primary", []):
        if primary.get("create", False):
            ...
            try:
                logger.info(
                    'Creating primary index on collection "%s" in scope "%s".',
                    collection,
                    scope,
                )
                ...
                logger.error(
                    'Scope "%s" not found for collection "%s".', scope, collection
                )
            except CouchbaseException as e:
                logger.error(
                    'Error creating primary index on collection "%s" in scope "%s": %s',
                    collection,
                    scope,
                    e,
                )
    ...
    for index in INDEXES.get("secondary", []):
        if index.get("create", False):
            ...
            try:
                logger.info(
                    'Creating secondary index "%s" on collection "%s" in scope "%s".',
                    index_name,
                    collection,
                    scope,
                )
                ...
                logger.error(
                    'Error creating secondary index "%s" on collection "%s" in scope "%s": %s',
                    index_name,
                    collection,
                    scope,
                    e,
    ...
    for fts_index in INDEXES.get("fts", []):
        if fts_index.get("create", False):
            ...
            fts_index_spec = {
                ...
                "planParams": {"indexPartitions": 6},
                "params": {
                    "doc_config": {
                        "mode": (
                            "scope.collection" if scope and collection else "bucket"
                        ),
                        "scope_name": scope,
                        "collection_name": collection,
                    },
            ...
            try:
                logger.info(
                    'Creating FTS index "%s" on collection "%s" in scope "%s".',
                    fts_index_name,
                    collection,
                    scope,
                )
        ...
        for scope_config in SCOPES:
            if scope_config.get("create", False):
                create_scope_and_collections(
                    collection_manager,
                    scope_config["name"],
                    scope_config["collections"],
                )

        create_indexes(cluster)

        logger.info(
            "All bucket, scopes, collections, and indexes created successfully!"
        )
